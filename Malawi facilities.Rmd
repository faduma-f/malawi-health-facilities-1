...
plot_facility_types_MFL = plot_facility_types_MFL + labs(x = "Facility types", y = "Frequency", fill="Ownership") + scale_fill_brewer(palette = "Set2") + coord_flip() + theme_pubclean() + ggtitle("MFL") + theme(axis.title.y = element_blank(), legend.title = element_text(size = 6), legend.text = element_text(size = 6), plot.title = element_text(face = "bold", hjust = 0), legend.position = "bottom")
...
ownership_MFL$Var1 = stringr::str_wrap(ownership_MFL$Var1, 16)

## bar plot of ownership
plot_ownership_MFL = ggplot(ownership_MFL, aes(x=Var1, y=Freq)) + geom_bar(stat = "identity", fill="slategray")
plot_ownership_MFL = plot_ownership_MFL + labs(x="Ownership", y = "Frequency") + coord_flip() + theme_pubclean() + ggtitle("MFL") + theme(axis.title.y = element_blank(), plot.title = element_text(face = "bold"))
plot_ownership_MFL
```
...
plot_facility_types_WHO = plot_facility_types_WHO + labs(x = "Facility types", y = "Frequency", fill="Ownership") + scale_fill_brewer(palette = "Set2") + coord_flip() + theme_pubclean() + ggtitle("WHO") + theme(axis.title.y = element_blank(), legend.title = element_text(size = 6), legend.text = element_text(size = 6), plot.title = element_text(face = "bold", hjust = 0), legend.position = "bottom") + expand_limits(y=c(0,600))
...
plot_RC_facility_types_WHO = plot_RC_facility_types_WHO + labs(x = "Reclassified facility types", y = "Frequency") + coord_flip() + theme_pubclean() + theme(plot.title = element_text(face = "bold")) + ggtitle("WHO") + expand_limits(y=c(0,600))
...
plot_ownership_WHO = plot_ownership_WHO + labs(x="Ownership", y = "Frequency") + coord_flip() + theme_pubclean() + ggtitle("WHO") + theme(axis.title.y = element_blank(), plot.title = element_text(face = "bold")) + expand_limits(y=c(0,600))
plot_ownership_WHO
```
...
tmap_facilities_WHO = tmap_admin2 + tm_shape(map_static_WHO) + tm_dots(col = "facility_type_9", palette = "viridis") + tm_layout(frame = FALSE, asp = 2, title = "WHO", title.position = c("left", "top"))
tmap_facilities_WHO
...
malawi_facilities_MFL$region_admin1 = lengths(admin1_facility_MFL) # gives error, no. of rows are less in                                                                          admin1_, why?
malawi_facilities_MFL$region_admin1 <- NULL
...
intersect_admin1_MFL = intersect_admin1_MFL[-c(10:15)] # remove columns 
intersect_admin1_MFL

# adding region data for admin1 to intersect_admin1_MFL (using admin1_facility_MFL from previously)
intersect_admin1_MFL$region_admin1 = admin1_facility_MFL
intersect_admin1_MFL 
...
no_intersect_admin1_MFL

# input 0s in the region_admin1 column
no_intersect_admin1_MFL$region_admin1 = rep.int(0, times = 61)
...
# bind data frames together to get new data frame with admin1 regions assigned correctly
malawi_facilities_MFL_admin1 = rbind(intersect_admin1_MFL, no_intersect_admin1_MFL)
tail(malawi_facilities_MFL_admin1) # !!! something is wrong, facilities with same district show different 
                                   # value for region. Was data for district inputted incorrectly or are 
                                   # coordinates wrong? Carry on for now.
                    
# use new data frame for plots
...
## transform columns
malawi_facilities_MFL_admin1$region_admin1 = as.factor(malawi_facilities_MFL_admin1$region_admin1)
malawi_facilities_MFL_admin1$TYPE = as.factor(malawi_facilities_MFL_admin1$TYPE)

## freq of facility types by region
df_facility_admin1_MFL = as.data.frame(table(malawi_facilities_MFL_admin1$region_admin1, 
                                             malawi_facilities_MFL_admin1$TYPE))
df_facility_admin1_MFL

## subsetting by region for plots 

# region 1 = Central 
admin1_region1 = filter(df_facility_admin1_MFL, Var1 == 1)

admin1_region1_plot = ggplot(admin1_region1, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "Central Region") + theme(plot.title = element_text(face = "bold")) + theme_pubclean() + coord_flip() + expand_limits(y=c(0, 250))
admin1_region1_plot

# region 2 = Southern
admin1_region2 = filter(df_facility_admin1_MFL, Var1 == 2)

admin1_region2_plot = ggplot(admin1_region2, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "Southern Region") + theme(plot.title = element_text(face = "bold")) + theme_pubclean() + coord_flip()
admin1_region2_plot

# region 3 = Northern
admin1_region3 = filter(df_facility_admin1_MFL, Var1 == 3)

admin1_region3_plot = ggplot(admin1_region3, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "Northern Region") + theme(plot.title = element_text(face = "bold")) + theme_pubclean() + coord_flip() + expand_limits(y=c(0, 250))
admin1_region3_plot
...
malawi_admin2 # 27 districts 
sum(malawi_admin2$facility) # also has 1365 intersections
malawi_admin2$facility # Lilongwe = 163 facilities
                       # Blantyre = 154 
                       # Mzimba = 95
...
df_facility_admin1_WHO

# Central
admin1_central = filter(df_facility_admin1_WHO, Var1 == "Central")

admin1_central_plot = ggplot(admin1_central, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "Central Region") + theme(plot.title = element_text(face = "bold")) + theme_pubclean() + coord_flip() + expand_limits(y=c(0, 200))
admin1_central_plot

# Southern
admin1_southern = filter(df_facility_admin1_WHO, Var1 == "Southern")

admin1_southern_plot = ggplot(admin1_southern, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "Southern Region") + theme(plot.title = element_text(face = "bold")) + theme_pubclean() + coord_flip()
admin1_southern_plot

# Northern
admin1_northern = filter(df_facility_admin1_WHO, Var1 == "Northern")

admin1_northern_plot = ggplot(admin1_northern, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "Northern Region") + theme(plot.title = element_text(face = "bold")) + theme_pubclean() + coord_flip() + expand_limits(y=c(0, 200))
admin1_northern_plot
...
malawi_admin2$facility_WHO # Mzimba = 56 facilities
                           # Lilongwe = 52 
                           # Mangochi = 45 
```
