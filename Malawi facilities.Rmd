...
intersect_admin1_MFL = intersect_admin1_MFL[-c(10:16)] # remove columns 
...
admin1_region1_plot = ggplot(admin1_region1, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "MFL Central Region") + theme(plot.title = element_text(face = "bold")) + theme_minimal() + coord_flip() + expand_limits(y=c(0, 250))

# region 2 = Southern
admin1_region2 = filter(df_facility_admin1_MFL, Var1 == 2)

admin1_region2_plot = ggplot(admin1_region2, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "MFL Southern Region") + theme(plot.title = element_text(face = "bold")) + theme_minimal() + coord_flip() + expand_limits(y=c(0,250))

# region 3 = Northern
admin1_region3 = filter(df_facility_admin1_MFL, Var1 == 3)

admin1_region3_plot = ggplot(admin1_region3, aes(x=Var2, y=Freq)) + geom_bar(stat = "identity", fill="slategray") + labs(x="Facility type", y="Frequency", title = "MFL Northern Region") + theme(plot.title = element_text(face = "bold")) + theme_minimal() + coord_flip() + expand_limits(y=c(0, 150))
...
```
   ...
   - Use satellite imagery to determine size of building and use that to sort into tiers (downside = many ways this can go wrong, what would the criteria be?)
   
```{r}
# Function that recognises different facility types 
sort_facilities = function(data, facility_column) {
   for(i in levels(facility_column)) {
      if(grepl("Central", facility_column, fixed = TRUE) == TRUE){
         output = grep("Central", facility_column, fixed = TRUE)
         t4 = slice(data, c(output[]))
      }
  return(t4)
 }
}
