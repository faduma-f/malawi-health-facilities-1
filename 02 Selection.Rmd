...
The data downloaded contained 1546 facilities in the MFL, 648 in the WHO-KWTRP list for Malawi and 248 in healthsites.io. Both the MFL and WHO-KWTRP contain some missing coordinates. Upon inspection, there are 181 missing values in the MFL, which also contains 62 coordinates that are not in Malawi and were either inputted incorrectly or not known. In the WHO-KWTRP, there are 9 missing coordinates and this was stated in (Maina et al., 2019). Healthsites.io carries some missing attributes but all coordinates are present and reside within Malawi. Assessing attribute data, the WHO-KWTRP contains the least, only describing ownership and first administrative regions the facilities reside in besides the core information of name, type and location. The MFL builds on this by including second level administrative regions, which are the districts in this case, functionality, date opened and alternative facility names under the column 'Common name'. However, the website that hosts the MFL gives the option of obtaining more information on capacity and resources by selecting an individual facility. Unfortunately this aspect is not available as part of the MFL data to download. Healthsites.io by far provides a greater variety of attributes out of the 3 sources. Although for Malawi, 14% of facilities have no names assigned, which is one of the basic elements required. Other attributes are also sparse.  
...
which(malawi_rhs$name == "Misuku Health Centre") # 3 85
which(malawi_rhs$name == "AREA18 mediclinic") # 36 48
which(malawi_rhs$name == "Leo Pharmacy") # 45 65
which(malawi_rhs$name == "Care Polyclinic Limited") # 50 60
which(malawi_rhs$name == "Likuni Drug Store") #  56 109
which(malawi_rhs$name == "Nakalanzi Health Centre") # 66 148
which(malawi_rhs$name == "Pharmcare") # 70 95
which(malawi_rhs$name == "Mwalingo Private Hospital") # 103 186
which(malawi_rhs$name == "Nsambe Rural Health Center") # 106 210
which(malawi_rhs$name == "Good hope pvt clinic") # 126 157
which(malawi_rhs$name == "Chancellor College Clinic") # 165 176
which(malawi_rhs$name == "Lifuwu Health Center") # 183 212
which(malawi_rhs$name == "Nkombezi Clinic") # 215 246
which(malawi_rhs$name == "Nkhoma Eye Clinic") #54 78

malawi_rhs = malawi_rhs[-c(3, 36, 45, 50, 56, 66, 70, 103, 106, 126, 165, 183, 215, 54), ]
```
...
# plot
plot_3sources = ggplot(number_3sources, aes(x=Sources, y=Number)) + geom_bar(stat = "identity", width = 0.7, fill="slategray") + labs(x = "Source of data", y = "Frequency") + theme_minimal() + expand_limits(y=c(0,1500)) + coord_flip()
plot_3sources
```  
...
```{r Maps, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Locations of facilities in the MFL and WHO-KWTRP."}
...
```  
  ...
  Comparison between private faciltiies is also made here, as the WHO-KWTRP data does not include this. In the MFL, 30% of all facilities are privately owned and majority of these consist of 356 clinics, 45 dispensaries and 16 hospitals. Distribution across the country indicates many are present in Blantyre and Lilongwe, with 95 and 67 respectively (figure 4).  
  
```{r Private MFL, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Number of private facilities per district obtained from the MFL."}
...
tmap_private_district_MFL = tm_shape(malawi_admin2) + tm_borders() + tm_shape(malawi_admin2) + tm_fill("private_MFL",  palette = sequential_hcl(6, palette = "YlGnBu", rev = TRUE), title = "Private facilities")
...
plot_hosp_3sources = ggplot(hosp_3sources, aes(x=Source2, y=Hospitals)) + geom_bar(stat = "identity", width = 0.7, fill="slategray") + labs(x = "Source of data", y = "Frequency") + theme_minimal() + expand_limits(y=c(0,200)) + coord_flip()
plot_hosp_3sources
...
# maps of blantyre 
tmap_blantyre_MFL =  tm_shape(blantyre_admin3) + tm_borders() + tm_shape(filter(malawi_admin2, shapeName == "Blantyre")) + tm_borders() + tm_shape(blantyre_intersect_admin2_MFL) + tm_dots(col = "TYPE", size = 0.3) + tm_layout(title = "MFL")


 # 154 facilities in total

tmap_blantyre_WHO =  tm_shape(blantyre_admin3) + tm_borders() + tm_shape(filter(malawi_admin2, shapeName == "Blantyre")) + tm_borders() + tm_shape(blantyre_intersect_admin2_WHO) + tm_dots(col = "Facility.type", size = 0.3) + tm_layout(title = "WHO")
...
tmap_private_blantyre = tm_shape(blantyre_admin3) + tm_borders() + tm_shape(filter(malawi_admin2, shapeName == "Blantyre")) + tm_borders() + tm_shape(blantyre_private_intersect_admin2_MFL) + tm_dots(col = "TYPE", size = 0.3) + tm_layout(title = "Private facilities", legend.outside = TRUE)
...
tmap_hosp_blantyre =  tm_shape(blantyre_admin3) + tm_borders() + tm_shape(filter(malawi_admin2, shapeName == "Blantyre")) + tm_borders() + tm_shape(blantyre_hosp_intersect_admin2_MFL) + tm_dots(col = "hospital", palette="red", title = "MFL hospitals", size = 0.3) + tm_shape(blantyre_hosp_intersect_admin2_rhs) + tm_dots(col = "amenity",title = "healthsites.io amenity tag", palette="darkgreen", size = 0.3) + tm_layout(title = "A")
...
tmap_hosp_health_blantyre =  tm_shape(blantyre_admin3) + tm_borders() + tm_shape(filter(malawi_admin2, shapeName == "Blantyre")) + tm_borders() + tm_shape(blantyre_hosp_intersect_admin2_MFL) + tm_dots(col = "hospital", palette="red", title = "MFL hospitals", size = 0.3) + tm_shape(blantyre_health_intersect_admin2_rhs) + tm_dots(col = "healthcare",palette="darkgreen", title = "healthsites.io healthcare tag", size = 0.3) + tm_layout(title = "B")
# 21 in healthsites 
