...
# Separate data.frame for tiers assigned to Malawi MFL 
...
tier_types_check = function(tier4_types,tier3_types,tier2_types,tier1_types,a,b,c,d){
 for(i in 1:4){
    
   types_in_other_tiers = c(rbind())
   
   for(i in tier4_types) {
     if(i %in% c[[1]] == TRUE){
     types_in_other_tiers = c[which(i == c[[1]]), ]
     }
     else if(i %in% b[[1]] == TRUE){
      types_in_other_tiers = b[which(i == b[[1]]), ]
     }
     else if(i %in% a[[1]] == TRUE){
      types_in_other_tiers = a[which(i == a[[1]]), ]
     }
     else{
      print("No tier 4 facility types present in other tiers")
     }
   return(tier4_in_other_tiers)
   tier4_types = tier3_types
